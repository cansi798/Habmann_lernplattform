...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  ...
  arrow/.style={-{Stealth[length=2.5mm]}, lpAccent, very thick}]
  \node[step, fill=lpAccentLight] (r1) at (0,0)
    {\textbf{Release 1 -- Attended}\\\scriptsize 6 Wochen, 60\,\% Prozess\\\scriptsize Vault-Logging};
  \node[gate] (g) at (5.0,1.2)
    {\scriptsize Excp.\,$\leq$\,12\,\%\\\scriptsize UI\,$\leq$\,1\,/\,M};
  \node[step, fill=lpGreenLight] (r2) at (9.5,2.4)
    {\textbf{Release 2 -- Unattended}\\\scriptsize Wo 7--12, 60\,\% Prozess\\\scriptsize SoD, Exception Queue};
  \node[step, fill=lpGrayLight, draw=lpGray] (loop) at (4.6,-1.8)
    {\scriptsize attended +2 Wochen\\\scriptsize (KPIs nachschärfen)};
  \draw[arrow] (r1.north east) to[bend left=10] (g.west);
  \draw[arrow] (g.east) to[bend left=10] node[above, font=\sffamily\scriptsize, lpAccent] {ja} (r2.west);
  \draw[arrow, dashed, lpGray] (g.south) -- ++(0,-0.5) -| (loop.north)
    node[midway, above, font=\sffamily\scriptsize, lpGray] {nein};
  \draw[arrow, dashed, lpGray] (loop.east) -- ++(1.0,0) |- (g.south west);
\end{tikzpicture}
\caption{\sffamily Stufenmodell Attended\,\textrightarrow\,Unattended -- Übergang nur über das Readiness-Gate.}
\end{figure}
\end{loesungbox}
