...
\ytvideo{Vertriebsmarketing -- klassisch, digital, plattformbasiert}{https://studyflix.de/wirtschaft/vertriebsmarketing-1229/video}
...
\ytvideo{Externe Effekte -- positive und negative Externalitäten}{https://studyflix.de/wirtschaft/externe-effekte-857/video}
...
\ytvideo{Indirekter Vertrieb -- Absatzmittler und Vertriebswege}{https://studyflix.de/wirtschaft/indirekter-vertrieb-1223/video}
...
\begin{tikzpicture}[font=\sffamily\footnotesize]
  % Heatmap der Bewertungsmatrix (Skala 1-5)
  \def\optnames{{"A: Amazon","B: Webshop","C: LinkedIn","D: Marktplatz"}}
  \def\critnames{{"Reichweite","Kostenmodell","Kontrollgrad","Datenzugang"}}
  % Daten Zeile 0..3 = Reichweite, Kostenmodell, Kontrollgrad, Datenzugang
  \def\matval{{{5,2,4,3},{3,2,3,3},{1,5,4,2},{1,5,3,2}}}
  % Spalten-Header (Optionen)
  \foreach \c in {0,1,2,3} {
    \pgfmathparse{\optnames[\c]}
    \node[anchor=south, align=center, text width=2cm] at (\c*1.6+0.8, 4*0.85+0.05) {\bfseries\pgfmathresult};
  }
  % Zeilen-Header (Kriterien) + Zellen
  \foreach \r in {0,1,2,3} {
    \pgfmathparse{\critnames[\r]}
    \node[anchor=east] at (-0.1, (3-\r)*0.85+0.4) {\bfseries\pgfmathresult};
    \foreach \c in {0,1,2,3} {
      \pgfmathsetmacro{\v}{\matval[\r][\c]}
      \pgfmathsetmacro{\mix}{(\v-1)*25}% 0..100
      \fill[lpAccent!\mix!lpGrayLight, draw=lpGray!50] (\c*1.6, (3-\r)*0.85) rectangle ++(1.55, 0.8);
      \pgfmathparse{\v<3 ? "lpInk" : "white"}
      \node[font=\sffamily\bfseries, color=\pgfmathresult] at (\c*1.6+0.775, (3-\r)*0.85+0.4) {\pgfmathprintnumber[fixed,precision=0]{\v}};
    }
  }
  % Summenzeile
  \node[anchor=east] at (-0.1, -0.5) {\bfseries Summe};
  \foreach \c/\s in {0/10,1/14,2/14,3/10} {
    \pgfmathparse{(\s>=14) ? "lpGreen" : ((\s<=10) ? "lpAmberBorder" : "lpGray")}
    \node[font=\sffamily\bfseries, color=\pgfmathresult] at (\c*1.6+0.775, -0.5) {\s};
  }
  % Legende
  \node[anchor=west, font=\sffamily\scriptsize\itshape, color=lpGray] at (0, -1.2) {Helle Felle = schwache Bewertung (1) \textbullet{} dunkle Felder = starke Bewertung (5)};
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[font=\sffamily\footnotesize,
  phase/.style={rectangle, rounded corners=3pt, draw=lpAccent, fill=lpAccentLight, text=lpInk,
    minimum width=2.2cm, minimum height=1.0cm, align=center, font=\sffamily\bfseries\footnotesize},
  tp/.style={rectangle, rounded corners=2pt, draw=lpGray!60, fill=white, text=lpInk,
    minimum width=2.2cm, minimum height=0.9cm, align=center, font=\sffamily\scriptsize,
    text width=2.0cm},
  arr/.style={-{Stealth[length=4pt]}, line width=0.8pt, color=lpAccent}]
  % Phasen-Pfeile
  \node[phase] (p1) at (0,0) {Awareness};
  \node[phase] (p2) at (2.6,0) {Consider.};
  \node[phase] (p3) at (5.2,0) {Decision};
  \node[phase] (p4) at (7.8,0) {Retention};
  \node[phase] (p5) at (10.4,0) {Advocacy};
  \draw[arr] (p1) -- (p2);
  \draw[arr] (p2) -- (p3);
  \draw[arr] (p3) -- (p4);
  \draw[arr] (p4) -- (p5);
  % Touchpoints darunter
  \node[tp, below=0.45cm of p1] (t1) {Google-\\Suche};
  \node[tp, below=0.45cm of p2] (t2) {Landingpage\\\& LinkedIn};
  \node[tp, below=0.45cm of p3] (t3) {Beratungs-\\termin};
  \node[tp, below=0.45cm of p4] (t4) {Kundenkonto\\Nachbest.};
  \node[tp, below=0.45cm of p5] (t5) {Referenz-\\Anfrage};
  \foreach \p/\t in {p1/t1,p2/t2,p3/t3,p4/t4,p5/t5} {
    \draw[densely dotted, color=lpGray] (\p) -- (\t);
  }
  % Conversion-Punkte
  \node[font=\sffamily\scriptsize\itshape, color=lpAmberBorder, below=0.15cm of t1] {Klick};
  \node[font=\sffamily\scriptsize\itshape, color=lpAmberBorder, below=0.15cm of t2] {Anfrage};
  \node[font=\sffamily\scriptsize\itshape, color=lpAmberBorder, below=0.15cm of t3] {Vertrag};
  \node[font=\sffamily\scriptsize\itshape, color=lpAmberBorder, below=0.15cm of t4] {Kauf};
  \node[font=\sffamily\scriptsize\itshape, color=lpAmberBorder, below=0.15cm of t5] {Freigabe};
\end{tikzpicture}
\end{center}
