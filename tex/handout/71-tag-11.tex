...
\begin{itemize}
  ...
  \item Nutzen und Risiken von RPA aus Senior-Brille einordnen: Schatten-IT (heimlich gebaute, unkontrollierte IT-Systeme), Credential-Risiko, fragile UI-Automation (UI = User Interface, Benutzeroberfl\"ache), Audit Trail.
\end{itemize}
...
\begin{itemize}
  ...
  \item Eine Stufenstrategie attended (Bot mit Mensch am Arbeitsplatz) $\to$ unattended (Bot auf Server, autonom) entwerfen, inkl.\ Readiness-KPIs (Key Performance Indicators -- Mess-Kennzahlen).
...
\begin{itemize}
  ...
  \item Security- und Compliance-Guardrails (Least Privilege = Minimalrechte-Prinzip, SoD = Segregation of Duties, also Funktionstrennung; Vault = zentraler verschl\"usselter Passwort-Tresor) als Pflichtprogramm setzen.
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  step/.style={rectangle, rounded corners=2pt, draw=lpAccent, thick, align=center,
               minimum width=3.6cm, minimum height=1.1cm},
  arrow/.style={-{Stealth[length=3mm]}, lpAccent, very thick}]
  \node[step, fill=lpAccentLight] (s1) at (0,0) {\textbf{Stufe 1 -- Attended-Pilot}\\\scriptsize Lernkurve, Akzeptanz, Stabilität};
  \node[step, fill=lpAmberLight, right=0.6cm of s1, yshift=0.9cm] (s2)
    {\textbf{Stufe 2 -- Hybrid}\\\scriptsize unattended skaliert, attended assistiert};
  \node[step, fill=lpGreenLight, right=0.6cm of s2, yshift=0.9cm] (s3)
    {\textbf{Stufe 3 -- Portfolio + CoE}\\\scriptsize Self-Service, Governance, Self-Service};
  \draw[arrow] (s1.north east) -- (s2.south west);
  \draw[arrow] (s2.north east) -- (s3.south west);
  % vertical axis hint
  \draw[-{Stealth[length=2mm]}, lpGray, thick] (-1.0,-0.4) -- (-1.0,2.4)
    node[midway, sloped, above, font=\sffamily\scriptsize\itshape] {Skalierung\,/\,Governance-Reife};
  \draw[-{Stealth[length=2mm]}, lpGray, thick] (-1.0,-0.4) -- (8.4,-0.4)
    node[midway, below, font=\sffamily\scriptsize\itshape] {Zeit\,/\,Plattformreife};
\end{tikzpicture}
\caption{\sffamily RPA-Stufenmodell -- Attended-Pilot \textrightarrow\ Hybrid \textrightarrow\ skaliertes Portfolio.}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{SLA-Stabilität.} (SLA = Service Level Agreement, vereinbarte Qualit\"ats- und Leistungsstandards.) Durchsatz wird vorhersagbar, Spitzen werden ausgeglichen.
