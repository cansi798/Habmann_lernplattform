...
\begin{itemize}
  ...
  \item TDD-Zyklus (Test-Driven Development -- Red-Green-Refactor: erst Test, dann Code) und Testpyramide (Unit, Integration, End-to-End) als Qualitätsarchitektur verstehen.
...
\begin{itemize}
  ...
  \item Ein Operating Model entwerfen, das Produktteams, Plattformteam und Security in einer RACI (Responsible, Accountable, Consulted, Informed -- Verantwortungs-Matrix) darstellt.
\end{itemize}

\subsection*{L3 -- Management (Entscheidung und Verantwortung)}
\begin{itemize}
  \item Top-10-Entscheidungen für eine skalierbare Plattform priorisieren (Servicegrenzen, Datenhoheit, SLOs = Service Level Objectives, messbare Ziel-Niveaus; Standards).
  \item Zielkonflikte Speed vs.\ Safety, Autonomie vs.\ Standardisierung, Innovation vs.\ Audit transparent entscheiden.
  \item Reliability- und Risiko-Architektur (SLOs, Error Budgets, Change Failure Rate, MTTR = Mean Time to Recover, mittlere Wieder\-herstellungs-Zeit) als Steuerungsinstrument nutzen.
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  mono/.style={rectangle, rounded corners=2pt, draw=lpGray, fill=lpGrayLight,
               minimum width=3.2cm, minimum height=2.6cm, align=center, thick},
  svc/.style={rectangle, rounded corners=2pt, draw=lpAccent, fill=lpAccentLight,
              minimum width=1.5cm, minimum height=0.8cm, align=center, thick},
  bus/.style={rectangle, draw=lpAccent, fill=lpGreenLight,
              minimum width=4.6cm, minimum height=0.4cm, align=center, thick},
  arrow/.style={-{Stealth[length=2.2mm]}, lpGray, thick, dashed}]
  % Monolith
  \node[mono] (mono) {\textbf{Monolith}\\[2pt]
    {\footnotesize UI\,/\,Logik\,/\,DB}\\
    {\footnotesize ein Deploy\\ein Team}};
  \node[below=0.05cm of mono, font=\sffamily\scriptsize\itshape, lpGray] {ein Stück, eine Grenze};
  % Right side: microservices with bus
  \node[svc, right=2.2cm of mono.north east, anchor=north west] (s1) {Bestellung};
  \node[svc, right=0.25cm of s1] (s2) {Bonität};
  \node[svc, right=0.25cm of s2] (s3) {Versand};
  \node[bus, below=0.5cm of s2] (bus) {\textsf{\scriptsize Event Bus / API Gateway}};
  \draw[arrow] (s1.south) -- (s1.south |- bus.north);
  \draw[arrow] (s2.south) -- (s2.south |- bus.north);
  \draw[arrow] (s3.south) -- (s3.south |- bus.north);
  \node[below=0.05cm of bus, font=\sffamily\scriptsize\itshape, lpGray] {viele Services, klare Grenzen};
  \draw[-{Stealth[length=2.5mm]}, lpAccent, very thick] (mono.east) -- ($(mono.east)+(1.4,0)$) node[midway, above, font=\sffamily\scriptsize] {Schnitt entlang};
  \node[font=\sffamily\scriptsize, lpAccent] at ($(mono.east)+(1.4,-0.32)$) {Bounded Context};
\end{tikzpicture}
\caption{\sffamily Vom Monolithen zu Microservices -- Schnitt entlang fachlicher Bounded Contexts.}
\end{figure}
