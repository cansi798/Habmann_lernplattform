...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  layer/.style={rectangle, rounded corners=2pt, draw=lpAccent, line width=0.4pt, minimum height=10mm, minimum width=110mm, align=center, inner sep=4pt},
  src/.style={rectangle, rounded corners=2pt, draw=lpGray, fill=lpGrayLight, line width=0.3pt, minimum height=7mm, minimum width=22mm, align=center, font=\sffamily\scriptsize},
  use/.style={rectangle, rounded corners=2pt, draw=lpAmberBorder, fill=lpAmberLight, line width=0.3pt, minimum height=7mm, minimum width=22mm, align=center, font=\sffamily\scriptsize},
  arr/.style={->, >=Stealth, draw=lpGray, line width=0.4pt}]
  \node[src] (s1) {Web / Forms};
  \node[src, right=3mm of s1] (s2) {E-Mail / MAS};
  \node[src, right=3mm of s2] (s3) {Telefon / Call};
  \node[src, right=3mm of s3] (s4) {Events / Messen};
  \node[src, right=3mm of s4] (s5) {Partner / Affiliate};
  \node[layer, fill=lpAccentLight, below=6mm of s3] (L1) {\textbf{Daten-Schicht} \;|\; Kontakte, Konten, Aktivitäten, Pflichtfelder};
  \node[layer, fill=lpAccentLight, below=3mm of L1] (L2) {\textbf{Prozess-Schicht} \;|\; Lead $\to$ Opportunity $\to$ Angebot $\to$ Abschluss};
  \node[layer, fill=lpAccent, text=white, below=3mm of L2] (L3) {\textbf{Steuerungs-Schicht} \;|\; Pipeline, Forecast, KPIs, Eskalationen};
  \node[use, below=6mm of L3, xshift=-44mm] (u1) {Vertrieb};
  \node[use, right=3mm of u1] (u2) {Marketing};
  \node[use, right=3mm of u2] (u3) {Service};
  \node[use, right=3mm of u3] (u4) {Leitung / GF};
  \foreach \x in {s1,s2,s3,s4,s5} { \draw[arr] (\x) -- (L1.north); }
  \draw[arr] (L1) -- (L2);
  \draw[arr] (L2) -- (L3);
  \foreach \x in {u1,u2,u3,u4} { \draw[arr] (L3.south) -- (\x.north); }
\end{tikzpicture}
\caption{\sffamily CRM als Steuerungsplattform in drei Schichten: Daten aus allen Kanälen werden zu Prozessen verdichtet und liefern die Steuerungssicht für Vertrieb, Marketing, Service und Leitung.}
\label{fig:crm-22t13}
\end{figure}
