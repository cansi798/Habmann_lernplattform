...
\begin{itemize}
  ...
  \item Conformance-Abweichungen klassifizieren: fehlende Schritte, zusätzliche Schritte, falsche Reihenfolge, Schleifen, SLA-Verletzungen (\emph{Service Level Agreement} -- vertraglich vereinbarte Qualitäts- und Leistungsstandards für Dienstleistungen).
...
\begin{itemize}
  \item Aus einem Mini-Event-Log Happy Path (idealer Standardablauf), Varianten und Engpässe ableiten -- ohne Tool.
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  t/.style={rectangle, rounded corners=2pt, draw=lpGray, line width=0.5pt,
    minimum width=12mm, minimum height=7mm, align=center, inner sep=1pt},
  tSoll/.style={t, fill=lpAccentLight, draw=lpAccent},
  tIst/.style={t, fill=lpGreenLight, draw=lpGray},
  tDev/.style={t, fill=lpAmberLight, draw=lpAmberBorder},
  arr/.style={-{Latex[length=1.8mm]}, draw=lpGray, line width=0.5pt},
  lbl/.style={font=\sffamily\scriptsize\bfseries, color=lpAccent},
  dev/.style={font=\sffamily\scriptsize\itshape, color=lpAmberBorder, align=center}]
  % Soll
  \node[lbl] at (-0.5,1.5) {Soll-Modell};
  \node[tSoll] (s1) at (0.6,0.8) {A};
  \node[tSoll] (s2) at (2.2,0.8) {B};
  \node[tSoll] (s3) at (3.8,0.8) {C};
  \node[tSoll] (s4) at (5.4,0.8) {D};
  \draw[arr] (s1) -- (s2); \draw[arr] (s2) -- (s3); \draw[arr] (s3) -- (s4);
  % Ist
  \node[lbl] at (-0.5,-1.0) {Ist (aus Log)};
  \node[tIst] (i1) at (0.6,-1.7) {A};
  \node[tDev] (i2) at (2.2,-1.7) {C};
  \node[tDev] (i3) at (3.8,-1.7) {X};
  \node[tIst] (i4) at (5.4,-1.7) {D};
  \draw[arr] (i1) -- (i2); \draw[arr] (i2) -- (i3); \draw[arr] (i3) -- (i4);
  % Annotation
  \node[dev] at (2.2,-2.5) {falsche\\Reihenfolge};
  \node[dev] at (3.8,-2.5) {zus.\ Schritt};
  \node[dev] at (2.9,0.0) {B fehlt};
\end{tikzpicture}
\caption{\sffamily Conformance Checking: Soll-Pfad gegen Ist-Pfad, Abweichungen markiert.}
\label{fig:konzept-71t07}
\end{figure}
...
\begin{itemize}
  ...
  \item Logs: nur ERP (\emph{Enterprise Resource Planning} -- integriertes Softwaresystem zur unternehmensweiten Ressourcenplanung) und E-Mail-Tickets verfügbar -- unvollständig.
