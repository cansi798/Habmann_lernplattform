...
\begin{itemize}
  ...
  \item Vier KPIs (\emph{Key Performance Indicators} -- Messgrößen zur Überwachung und Steuerung unternehmerischer Ziele; zwei Outcome/Speed, zwei Risk/Compliance) mit Ownern festlegen.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Erfolgskriterien:} DLZ-Median $<24\,\text{h}$; Compliance-Findings = 0; Kunden-NPS (\emph{Net Promoter Score} -- Kennzahl für Kundenloyalität: Weiterempfehlungsbereitschaft auf einer Skala 0--10) $>+30$.
\end{itemize}
...
\begin{itemize}
  ...
  \item Die konkreten \emph{Schnittstellen} (REST/gRPC/AsyncAPI -- \emph{API} = \emph{Application Programming Interface}, standardisierte Schnittstelle zur Kommunikation zwischen Softwaresystemen; Auth-Verfahren, Endpoints).
...
\begin{itemize}
  \item Konkrete Plattform: bestehendes IAM (\emph{Identity \& Access Management} -- System zur Verwaltung von Identitäten und Zugriffsrechten)-System plus Verifikations-Service eines Anbieters.
