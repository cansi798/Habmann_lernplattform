...
Sehen wir uns ein konkretes Beispiel an -- das Use Case Diagram, das wir später in der Fallstudie (Sektion 8) ausführlich nutzen. \emph{CloudWorks B2B} (B2B = \emph{Business-to-Business}, Geschäftsbeziehungen zwischen Unternehmen) ist ein SaaS-Anbieter, der seinen Onboarding-Prozess verbessern möchte.
...
Bewegung & Mitarbeiter laufen zwischen Stationen & System-Wechsel pro Vorgang (CRM=\emph{Customer Relationship Mgmt.} \textrightarrow{} Ticket \textrightarrow{} ERP=\emph{Enterprise Resource Planning} \textrightarrow{} Mail) \\
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  node distance=6mm,
  meth/.style={rectangle, rounded corners=2pt, draw=lpAccent, fill=lpAccentLight,
    line width=0.6pt, minimum width=30mm, minimum height=11mm, align=center, inner sep=2pt},
  proc/.style={circle, draw=lpGray, fill=lpGrayLight, line width=0.6pt,
    minimum size=22mm, align=center, inner sep=1pt},
  qa/.style={font=\sffamily\scriptsize\itshape, color=lpGray, align=center},
  arr/.style={-{Latex[length=2mm]}, draw=lpAccent, line width=0.5pt}]
  \node[proc] (p) {Prozess\\(Realität)};
  \node[meth, above=10mm of p] (uml) {UML\\\scriptsize Struktur \& Rollen};
  \node[meth, below left=8mm and 4mm of p] (vsm) {VSM\\\scriptsize Wertfluss \& Zeit};
  \node[meth, below right=8mm and 4mm of p] (pm) {Process Mining\\\scriptsize Datenrealität};
  \draw[arr] (uml) -- (p);
  \draw[arr] (vsm) -- (p);
  \draw[arr] (pm) -- (p);
  \node[qa, right=1mm of uml] {Wer? Was?};
  \node[qa, left=1mm of vsm] {Wo Zeit\\verloren?};
  \node[qa, right=1mm of pm] {Was passiert\\wirklich?};
\end{tikzpicture}
\caption{\sffamily Der Methodendreiklang: drei Perspektiven, ein Prozess.}
\label{fig:konzept-71t02}
\end{figure}
...
\begin{itemize}
  \item \emph{UML Use Case} -- klärt, dass es vier Vertragsarten gibt mit unterschiedlichen Akteuren% Korr: Tippfehler "Verträgsarten" -> "Vertragsarten" (Standardvertrag, Rahmenvertrag, Sondervertrag, Compliance-Sondercase).
...
\begin{enumerate}
  ...
  \item \fachbegriff{SLA-Eskalations\-regel beim Provisioning} (SLA = \emph{Service Level Agreement}, vertraglich vereinbarte Leistungsstandards) -- nach 48\,h ohne Fortschritt ein automatischer Eskalationstrigger plus Standardpakete für Zugänge (z. B. ``Onboarding-Basis-Paket''). Reduziert Wartezeit und Varianz.
\end{enumerate}
