...
\begin{itemize}
  ...
  \item Sustainment-Mechanik (Verstetigung, d.\,h.\ dauerhafte Verankerung der Veränderung) kennen: Routinen, KPIs, Reviews, Adoption-Messung (Nutzungsgrad).
\end{itemize}
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  cell/.style={rectangle, draw=lpAccent, thick, minimum width=3.2cm, minimum height=1.8cm, align=center},
  ax/.style={-{Stealth[length=2mm]}, lpGray, thick}]
  \node[cell, fill=lpGrayLight]   (q1) at (0,0)   {\textbf{Informieren}\\\scriptsize Einfluss \textendash\ , Betroff.\ \textendash};
  \node[cell, fill=lpAccentLight] (q2) at (3.3,0) {\textbf{Verantworten}\\\scriptsize Einfluss \textendash\ , Betroff.\ +};
  \node[cell, fill=lpAmberLight]  (q3) at (0,2)   {\textbf{Konsultieren}\\\scriptsize Einfluss +, Betroff.\ \textendash};
  \node[cell, fill=lpGreenLight]  (q4) at (3.3,2) {\textbf{Mitentscheiden}\\\scriptsize Einfluss +, Betroff.\ +};
  \draw[ax] (-1.9,-1.1) -- (-1.9,3.1) node[midway, sloped, above, font=\sffamily\scriptsize] {Einfluss};
  \draw[ax] (-1.9,-1.1) -- (5.2,-1.1) node[midway, below, font=\sffamily\scriptsize] {Betroffenheit};
\end{tikzpicture}
\caption{\sffamily Stakeholder-Matrix -- Einbindungsstufe je nach Einfluss und Betroffenheit.}
\end{figure}
...
\begin{itemize}
  \item \textbf{Pulse Survey} (kurze, regelmäßige Stimmungsumfrage). 3--5 Fragen, monatlich.
...
\begin{enumerate}
  \item \fachbegriff{Standards.} Was ist die neue ``Definition of Done'' (DoD, d.\,h.\ Abschlusskriterien)? Was wird wie protokolliert?
...
\begin{itemize}
  \item \fachbegriff{Outcome} (Ergebnis)\textbf{.} Hat sich das Geschäftsergebnis verbessert? (Kunde, Qualität, Kosten.)
  \item \fachbegriff{Prozess.} DLZ kürzer, Rework (Nacharbeit) geringer, SLA-Quote besser?
  \item \fachbegriff{Adoption} (Nutzungsgrad)\textbf{.} Wieviele Fälle laufen im neuen Prozess? Wieviele umgehen ihn?
  \item \fachbegriff{Capability} (Fähigkeit)\textbf{.} Wie viele Mitarbeitende haben Training und Coaching abgeschlossen?
...
\section{Fallstudie: ServiceCo -- E2E-Prozessveränderung (End-to-End, d.\,h.\ vom Auftrag bis zur Abrechnung)}
