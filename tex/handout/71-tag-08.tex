...
\begin{itemize}
  ...
  \item ML-Typen (\emph{Machine Learning} -- Teilgebiet der KI: Systeme erkennen Muster in Daten und lernen daraus) im Prozesskontext unterscheiden: Klassifikation, Regression, Clustering, Anomalieerkennung.
  \item DWH/ETL-Grundlagen kennen: Fakten und Dimensionen, Staging, Mapping, Quality Checks.
  \item Dashboard-Prinzipien benennen: Executive-KPIs (\emph{Key Performance Indicator} -- Messgröße zur Überwachung und Steuerung unternehmerischer Ziele), Drilldown, Alerts, ``next best action''.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \fachbegriff{Predictive-Reifegrad.} Zukunft vorhersagen (Welche Cases werden in den nächsten 4\,h SLA -- \emph{Service Level Agreement}, vertraglich vereinbarte Qualitäts- und Leistungsstandards -- reißen?). Erlaubt \emph{präventives} Eingreifen.
\end{enumerate}
...
\begin{tikzpicture}[font=\sffamily\footnotesize,
  src/.style={rectangle, rounded corners=2pt, draw=lpGray, fill=lpGrayLight,
    line width=0.5pt, minimum width=14mm, minimum height=6mm, align=center, inner sep=1pt},
  stage/.style={rectangle, rounded corners=2pt, draw=lpAccent, fill=lpAccentLight,
    line width=0.6pt, minimum width=16mm, minimum height=9mm, align=center, inner sep=2pt},
  dwh/.style={cylinder, shape border rotate=90, aspect=0.25, draw=lpAccent,
    fill=lpGreenLight, line width=0.6pt, minimum width=18mm, minimum height=14mm,
    align=center, inner sep=1pt},
  bi/.style={rectangle, rounded corners=2pt, draw=lpAccent, fill=lpAccentLight,
    line width=0.6pt, minimum width=16mm, minimum height=9mm, align=center, inner sep=2pt},
  arr/.style={-{Latex[length=1.8mm]}, draw=lpAccent, line width=0.5pt}]
  \node[src] (s1) at (0,1.0)  {ERP};
  \node[src] (s2) at (0,0.0)  {CRM};
  \node[src] (s3) at (0,-1.0) {BPMS/Logs};
  \node[stage] (e) at (2.6,0) {ETL\\\scriptsize Extract\\Transform};
  \node[dwh]  (d) at (5.4,0) {DWH\\\scriptsize Fakten\\\& Dim.};
  \node[bi]   (b) at (8.2,0) {BI\\\scriptsize Dashboards};
  \draw[arr] (s1.east) -- (e.west);
  \draw[arr] (s2.east) -- (e.west);
  \draw[arr] (s3.east) -- (e.west);
  \draw[arr] (e) -- (d);
  \draw[arr] (d) -- (b);
\end{tikzpicture}
\caption{\sffamily Datenpfad von Quellsystemen über ETL und DWH zum BI-Dashboard.}
\label{fig:konzept-71t08}
\end{figure}

\subsection{Typische Quellsysteme}

\begin{itemize}
  \item \fachbegriff{ERP} (\emph{Enterprise Resource Planning} -- integriertes Softwaresystem zur unternehmensweiten Ressourcenplanung; Aufträge, Rechnungen, Stammdaten).
  \item \fachbegriff{CRM} (\emph{Customer Relationship Management} -- systematische Gestaltung aller Kundenbeziehungen im Unternehmen; Kunden, Kommunikation, Interaktionen).
...
\begin{itemize}
  ...
  \item \fachbegriff{Bearbeitungs\-zeit-Reduktion.} Risiko: ``Quick-Close'' ohne Lösung. Gegenmaßnahme: Kombination mit Kunden-NPS (\emph{Net Promoter Score} -- Kennzahl für Kundenloyalität: Weiterempfehlungsbereitschaft auf einer Skala 0--10).
\end{itemize}
...
\subsection{Schritt 6 -- MVP-Entscheidung (\emph{Minimum Viable Product} -- erste marktreife Produktversion mit minimalem Funktionsumfang)}
